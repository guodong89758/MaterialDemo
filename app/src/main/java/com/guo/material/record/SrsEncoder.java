...
public class SrsEncoder {
    ...
    public static final int VCROP_HEIGHT = 1080;
    ...
    public static final int ABITRATE = 2000000;  //
...
//                            vencoder.queueInputBuffer(inBufferIndex, 0, mRotatedFrameBuffer.length, pts, 0);
    ...
    private void portraitPreprocessYuvFrame(byte[] data) {
        mCroppedFrameBuffer = new byte[data.length];
        mFlippedFrameBuffer = new byte[data.length];
        mRotatedFrameBuffer = new byte[data.length];
        if (mCameraFaceFront) {
